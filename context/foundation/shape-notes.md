...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "workflow friction + energy/willpower depletion after study session"
    - topic: "insight"
      decision: "existing tools (Anki, Quizlet) require manual creation; LLMs now make auto-generation viable"
    - topic: "primary persona"
      decision: "professionals upskilling after hours"
    - topic: "SR algorithm depth"
      decision: "integrate existing open-source algorithm (SM-2 / FSRS) as must-have"
    - topic: "acceptance metric definition"
      decision: "edit-then-save counts as accepted for the 75% KPI"
    - topic: "product type"
      decision: "web app"
    - topic: "target scale"
      decision: "medium (dozens to a hundred users); domain rule is scale-independent"
    - topic: "non-goals"
      decision: "no file import, no sharing, no custom SR algorithm, no mobile, no offline"
  frs_drafted: 7
...
The insight: Anki and Quizlet still require card-by-card manual authoring and offer no AI generation. Recent LLM quality now makes it viable to auto-generate accurate, atomic Q&A pairs from pasted text — shifting the user's role from author to reviewer.
...
- **Authentication**: Login via email + password, OAuth, or passwordless — cards must persist across devices and sessions.
...
1. Professional registers with email + password / logs in.
2. Pastes a block of text from their study session.
3. AI generates a set of candidate flashcards.
4. User reviews, edits, and accepts (or rejects) each candidate card.
5. User starts a spaced repetition study session on their accepted cards.

## Timeline

`mvp_weeks: 3` — 3 weeks of after-hours work.
...
- FR-007: User can start a spaced repetition study session using an existing open-source algorithm (e.g. SM-2 / FSRS). Priority: must-have
  > Socrates: Counter-argument considered: "off-the-shelf SR libraries (e.g. ts-fsrs) exist; integration risk is low — the concern is the UI for the review loop itself." Resolution: the FR captures the capability and algorithm requirement; UI design is downstream. No counter-argument to the FR itself; stands as written.
...
- Business Logic: present (one-sentence rule + generation + scheduling)
