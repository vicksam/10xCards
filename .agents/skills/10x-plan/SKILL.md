...
2. **Spawn initial research tasks to gather context** (skip or narrow based on Step 1.0):
   Before asking the user any questions, create parallel sub-agents to research:
   - **Explore agent** (a specialized agent for file discovery and codebase structure questions) — find all files related to the task, search for patterns, trace code paths.
   - **general-purpose agent** (a general-purpose agent for deeper analysis) — for deeper analysis that may require reading many files and synthesizing findings. Use for understanding complex systems.
...
6. **Ask deep probing questions**:
   ...
   **Rules for structuring questions:**
   ...
   - Use multi-select only when choices aren't mutually exclusive
...
3. **Create research tasks** to track exploration (these appear in the user's status bar). Update them as research completes.
...
4. **Spawn parallel sub-tasks for comprehensive research**:
   ...
   **For codebase investigation:**
   - **Explore** (a specialized agent for fast file/pattern search, code structure analysis)
   - **general-purpose** (a general-purpose agent for deep analysis requiring multi-step reasoning)
...
5. **Track Progress**:
   - Create planning tasks and mark them completed as you progress
